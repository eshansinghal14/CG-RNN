...
from cgnet.cgnet.feature import SchnetFeature
...
class Simulation():
    ...
    Notes). The scheme chosen is BAOA(F)B, where
        ...
        F = force calculation (i.e., from the cgnet1)
    ...
    Parameters
    ----------
    model : cgnet1.network.CGNet() instance
    ...
    following reasons:
		...
		1) If cgnet1.network.Simluation is subclassed for multiple or specific
        model types, the _input_model_checks() method can be overridden without
        repeating a lot of code. As an example, see
        cgnet1.network.MultiModelSimulation, which uses all of the same
        simulation options as cgnet1.network.Simulation, but overides
        _input_model_checks() in order to perform the same model checks as
        cgnet1.network.Simulation but for more than one input model.

		2) If cgnet1.network.Simulation is subclassed for different simulation
    ...
    def calculate_potential_and_forces(self, x_old):
        ...
        cgnet1.network.MultiModelSimulation, where this method is overridden
...
class MultiModelSimulation(Simulation):
    ...
    see help(cgnet1.network.Simulation)

    Parameters
    ----------
    models: list of cgnet1.network.CGNet() instances
    ...
    Notes
    -----
    This class inherits from cgnet1.network.Simulation. The same checks for
    ...
    cgnet1.network.Simulation._input_option_checks
    """
